...
def build_logger(name):
    ...
    fh_formatter = logging.Formatter(
        '%(levelname)s(%(asctime)s) %(filename)s@%(lineno)d %(name)s(%(threadName)s) - %(message)s')
...
class M3U8Downloader(threading.Thread):
    def __init__(self, index_url: str, save_folder: str, ctx, time_per_video: int = 18000, dump_time=60,
                 folder_callback=None, limit_times=[RecordLimitData(1, 24 * 60 * 60)]):
        super().__init__()
    ...
    def single_loop(self, ):
        ...
        for segment in playlist.segments:
            ...
            if timestamp in self.ctx.TS_FILE_DICT.keys(): continue
            ...
            self.ctx.TS_FILE_DICT[timestamp] = segment.absolute_uri
